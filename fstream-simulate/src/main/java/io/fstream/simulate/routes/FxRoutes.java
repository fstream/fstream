...
public class FxRoutes extends AbstractRoutes {
  ...
  public void configure() throws Exception {
    ...
    from("timer://quote-timer1?daemon=false&period=" + period * 1)
      .process(new RandomQuoteEventGenerator("EUR/USD", 1.2757f, 1.3990f))
      .to(PUBLISH_ENDPOINT);
    from("timer://quote-timer2?daemon=false&period=" + period * 2)
      .process(new RandomQuoteEventGenerator("USD/JPY", 93.8675f, 105.4415f))
      .to(PUBLISH_ENDPOINT);
    from("timer://quote-timer3?daemon=false&period=" + period * 3)
      .process(new RandomQuoteEventGenerator("GBP/USD", 1.4817f, 1.6997f))
      .to(PUBLISH_ENDPOINT);
    from("timer://quote-timer4?daemon=false&period=" + period * 4)
      .process(new RandomQuoteEventGenerator("AUD/USD", 0.8661f, 0.9791f))
      .to(PUBLISH_ENDPOINT);
    from("timer://quote-timer5?daemon=false&period=" + period * 5)
      .process(new RandomQuoteEventGenerator("USD/CHF", 0.8701f, 0.9792f))
      .to(PUBLISH_ENDPOINT);
    from("timer://quote-timer6?daemon=false&period=" + period * 6)
