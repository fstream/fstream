...
/**
 * A price,time ordered implementation of a central limit order book. The
 * principle data structure is a List of IOrders - by definition only limit
 * orders can live in the book. Market orders are accepted and trigger a trade
...
public class OrderBook extends UntypedActor {
	...
	/**
	 * Accepts IOrder and executes it against available depth. Returns unfilled amount
	 * TODO currently MarketOrders are mimicked via LimitOrders where trigger price is best ask/bid. Need to add marketable order implementation
	 * @param order
	 * @return
	 */
	private  int processMarketOrder (LimitOrder order) {
		...
		while (bookiterator.hasNext()) {
			...
			while (orderiterator.hasNext()) {
				...
				if (order.getSide() == OrderSide.ASK) { // limit price exists, respect bounds
					if (order.getPrice() > passiveorder.getPrice()) {
						log.info(String.format("breaking price crossed on active ASK (SELL) MO for %s orderprice=%s passiveorder=%s",this.getSymbol(),order.getPrice(),passiveorder.getPrice()));
					...
					if (order.getPrice() < passiveorder.getPrice()) {
						log.info(String.format("breaking price crossed on active BID (BUY) MO for %s orderprice=%s passiveorder=%s",this.getSymbol(),order.getPrice(),passiveorder.getPrice()));
						this.updateDepth(order.getSide(), totalexecutedsize);
						this.updateBestPrices();
						return unfilledsize; // price has crossed
					}
				...
				if (unfilledsize == 0) {
					...
					orderiterator.remove(); // remove the passive order (last one returned by iterator)
					...
					// incoming larger than the first order in current level. keep
					// on iterating.
			...
			if (book.get(pricelevel.getKey()).isEmpty()) {
				bookiterator.remove(); // removes price level if orderqueue in it is empty (last one returned by iterator)
			}
		}		
		this.updateDepth(order.getSide(), totalexecutedsize);
		this.updateBestPrices();
	...
	/**
	 * Checks the validity of the book by inspecting actual depth in the book and comparing it to maintained biddepth/askdepth variables
	 * @return
	 */
	 ...
	 * @param executedsize
	 */
	private void registerTrade(IOrder active, IOrder passive, int executedsize) {
		tradecount += 1;
	...
	private  void addLimitOrder(LimitOrder order) {
		...
		int unfilledsize;
		if (crossesSpread(order) && availabledepth > 0) { // if limitprice crosses spread, treat as marketorder

			unfilledsize = processMarketOrder(order);
			order.setAmount(unfilledsize); // any unfilled amount added to orderbook
			if (unfilledsize > 0) {
				insertOrder(order);
			}
	...
	/**
	 * Determines if a limitorder crosses the spread i.e. LimitBuy is better priced than bestask or LimitSell is better priced than bestbid
	 * @param order
	 * @return
	 */
	...
	public static Comparator<LimitOrder> orderTimeComparator = new Comparator<LimitOrder>() {
		...
		public int compare(LimitOrder order1, LimitOrder order2) {
			...
			if (order1.getSentTime().getMillis() < order2.getSentTime().getMillis()) {
				...
				return -1; //TODO this is a hack so orders at same time are not ignored. need a better data structure for orders
			}
	...
	public void printBook() {
		...
		for (val ask : asks.entrySet()) {
			...
			for (val firstnode : ask.getValue()) {
				book = book + String.format("( %s,%s,%s) -> ", firstnode.getSentTime().toString(),firstnode.getPrice(),firstnode.getAmount());
			}
			book = book + "\n";
		}
		book = book + "------ BIDS -------\n";
		...
		book = book + String.format("best ask = %s, best bid =%s, spread = %s\n", this.bestask, this.bestbid, this.bestask-this.bestbid);
		book = book + "----- END -----\n";
		log.info(book);
	}
	...
	public void onReceive(Object message) throws Exception {
		...
		if (message instanceof IOrder) {
			this.processOrder((IOrder)message);
		}
			...
			if (message.equals(Messages.PRINT_ORDER_BOOK))  {
				...
				log.info(String.format("%s orders processed=%s, trades processed=%s, biddepth=%s, askdepth=%s bestask=%s bestbid=%s spread=%s", symbol,ordercount,tradecount,biddepth,askdepth,bestask,bestbid,bestask-bestbid));
			}
		}
		else {
			unhandled(message);
